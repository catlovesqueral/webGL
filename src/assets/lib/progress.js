...
Progress.prototype= {
    init:function (init) {
        ...
        let cvsElement = document.getElementById(this.el),//获取元素
            ...
            img = null;//背景图
        //停止时的角度
        init.degActive>0&&init.degActive<=100?
            this.degActive = init.degActive:this.degActive = 100;
        ...
        this.timer = function redraw(){
            ...
            ctx.arc(width/2,height/2,this.circleRadius,-Math.PI/2,this.degActive/100*Math.PI);
            ctx.strokeStyle=this.lineColor;
            ctx.stroke();
            ctx.drawImage(newImg,0,0)
            let txt=(this.degActive +"%");//获取百分比
